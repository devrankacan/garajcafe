...
export default function WaiterPage() {
  ...
  if (!authChecked)
    return (
      <div className="felt-bg min-h-screen flex items-center justify-center">
        <p className="animate-pulse" style={{ color: "#cc1515" }}>Yükleniyor...</p>
      </div>
    );
  ...
  return (
    <div className="felt-bg min-h-screen">
      {/* Header */}
      <header className="sticky top-0 z-20" style={{ background: "#080808", borderBottom: "1px solid rgba(204,21,21,0.3)" }}>
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center justify-between">
          <div>
            <h1 className="font-bold text-lg" style={{ color: "#cc1515" }}>♠ Garson Paneli</h1>
            ...
            {step !== "table" && (
              <>
                <button onClick={() => setStep("menu")}
                  ...
                  style={step === "menu" ? { background: "#cc1515", color: "#ffffff" } : { background: "rgba(204,21,21,0.12)", color: "#cc1515", border: "1px solid rgba(204,21,21,0.3)" }}>
                  Menü
                </button>
                <button onClick={() => setStep("cart")}
                  className="px-3 py-1.5 rounded-lg text-sm font-medium relative"
                  style={step === "cart" ? { background: "#cc1515", color: "#ffffff" } : { background: "rgba(204,21,21,0.12)", color: "#cc1515", border: "1px solid rgba(204,21,21,0.3)" }}>
                  Sepet
      ...
      <div className="max-w-2xl mx-auto px-4 py-4">
        ...
        {step === "table" && (
          <div>
            <h2 className="font-semibold mb-4" style={{ color: "#cc1515" }}>♦ Masa Seç</h2>
        ...
        {step === "menu" && (
          <div>
            <p className="text-sm mb-3" style={{ color: "#cc1515" }}>♣ Masa: <span className="font-bold text-white">{selectedTable?.name}</span></p>
              ...
              {activeProducts.map((product) => {
                ...
                return (
                  <div key={product.id} className="rounded-xl p-3 flex items-center justify-between"
                    style={{ background: "#1a1a1a", border: "1px solid rgba(204,21,21,0.2)" }}>
                    <div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => addToCart(product)} className="w-7 h-7 rounded-full font-bold flex items-center justify-center" style={{ background: "rgba(204,21,21,0.2)", color: "#cc1515" }}>+</button>
                    </div>
                  </div>
                );
              })}
        ...
        {step === "cart" && (
          <div>
            <h2 className="font-semibold mb-3" style={{ color: "#cc1515" }}>♥ Sipariş — {selectedTable?.name}</h2>
            {cart.length === 0 ? (
                  ...
                  {cart.map((item) => (
                    <div key={item.product.id} className="rounded-xl p-3 flex items-center justify-between"
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => addToCart(item.product)} className="w-7 h-7 rounded-full font-bold flex items-center justify-center" style={{ background: "rgba(204,21,21,0.2)", color: "#cc1515" }}>+</button>
                      </div>
                    </div>
                  ))}
                </div>
                <div className="rounded-xl p-3 mb-4" style={{ background: "#1a1a1a", border: "1px solid rgba(204,21,21,0.2)" }}>
                  <label className="block text-xs font-semibold uppercase mb-1.5" style={{ color: "#cc1515" }}>Not</label>
                  <textarea value={note} onChange={(e) => setNote(e.target.value)} rows={2}
                    className="w-full rounded-lg p-2 text-sm text-white resize-none focus:outline-none"
                    style={{ background: "rgba(0,0,0,0.3)", border: "1px solid rgba(204,21,21,0.2)" }}
                    placeholder="Örn: Az şekerli..." />
                </div>
                <div className="flex justify-between font-bold text-lg mb-4 px-1">
                  <span className="text-white">Toplam</span>
                  <span style={{ color: "#cc1515" }}>{cartTotal.toLocaleString("tr-TR", { style: "currency", currency: "TRY" })}</span>
                </div>
